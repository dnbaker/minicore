...
l1_sum_core(blz::SM<FT> &mat, std::string out, SumOpts &opts) {
    ...
    if(opts.discrete_metric_search) { // Use EM to solve instead of JV
        ...
        std::tie(indices, asn, costs) = repeatedly_get_initial_centers(mat, rng, opts.k, opts.kmc2_rounds, opts.extra_sample_tries, opts.lspp, opts.use_exponential_skips, blz::L1Norm());
    }
