...
| Total variation distance | geometric metric | TVD | 9 | Useful information theoretic measure; 1/2 L1 distance in probability space|
...
| Itakura-Saito Divergence | Bregman divergence | IS | 13 | Corresponds to the exponential distribution; sum((x / y) - log(x / y) - 1)|
...
| Reversed Itakura-Saito Divergence | Bregman divergence | `REVERSE_ITAKURA_SAITO` | 14 | Corresponds to the multinomial distribution with fixed n, or the categorical distribution; sum(y / x) - log(y / x) - 1|
| Symmetric Itakura-Saito Divergence | Bregman divergence | SIS | 23 | Corresponds to the multinomial distribution with fixed n, or the categorical distribution|
| Hellinger Distance | metric | HELLINGER | 6 | Information-theoretic measure with useful upper/lower bounds for TVD and JSD; sqrt(sum((sqrt(x) - sqrt(y))^2))|
| Bhattacharyya Metric | metric | `BHATTACHARYYA_METRIC` | 7 | Non-metric, but useful measure of dissimilarity. Related to the metric.; sqrt(1 - sum(sqrt(x) * sqrt(y))) |
| Bhattacharyya Distance | dissimilarity measure | `BHATTACHARYYA_DISTANCE` | 8 | Non-metric, but useful measure of dissimilarity. Related to the metric;  -log(sum(sqrt(x) * sqrt(y)))|
| LLR | dissimilarity measure | `LLR` | 10 | non-metric, but can be processed as a data-dependent rho-metric. As the prior rises, the $\rho$-violation of the triangle inequality shrinks; see below|
| UWLLR | dissimilarity measure | `UWLLR` | 12 | non-metric, but can be processed as a data-dependent rho-metric. As the prior rises, the $\rho$-violation of the triangle inequality shrinks. JSD and S2JSD LS hash functions will likely work for this and LLR; see below for formulation|
