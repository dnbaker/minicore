...
struct NaiveJVSolver {
    ...
    void setup(const MatType &mat) {
        ...
        pdqsort(&edges_[0], &edges_[edges_.size()], [](const auto x, const auto y) {return x.first > y.first;});
    ...
    std::pair<uint32_t, double> min_opening_cost() const {
        ...
        for(const auto fid: nottempopen_) {
            ...
            auto cost = nsupport ? (facility_cost_ - availsum) / nsupport: std::numeric_limits<double>::max();
            if(cost < mincost) mincost = cost, ind = fid;
        }
        return std::make_pair(ind, mincost);
    }
    void perform_increment(double inc,  std::vector<uint32_t> &to_remove) {
        maxalph_ += inc;
        for(const auto item: S) {
            v_[item] = maxalph_;
            for(size_t fi = 0; fi < w_.rows(); ++fi) {
                if(maxalph_ >= c[fid,s]) {
                    if(tempopen_.find(fid) != tempopen_.end()) // && std::find(to_remove.begin(), to_remove.end(), item) != to_remove.end())
                        to_remove.push_back(item);
                }
            }
            w_(fid, item) = std::max(0., maxalph_ - c(fid, item));
        }
    }
    template<typename MatType>
    void phase1(const MatType &mat) {
        ...
        while(S.size()) {
            ...
            perform_increment(inc, to_remove);
            if(!tighten) {
                auto fc = facility_cost_;
                tempopen_.insert(bestfac);
                nottempopen_.erase(bestfac);
                for(const auto item: S) {
                    if(v_[item] >= mat_(bestfac, item)) // && std::find(to_remove.begin(), to_remove.end(), s) != to_remove.end())
                        to_remove.push_back(item);
                }
            }
            for(const auto item: to_remove)
