...
int main(int argc, char *argv[]) {
    ...
    auto full_jsm = fgc::distance::make_jsm_applicator(filtered_sparsemat);
    fgc::util::Timer timer("k-means");
    auto kmppdat = fgc::distance::make_kmeanspp(full_jsm, k);
    timer.report();
    std::fprintf(stderr, "finished kmpp. Now getting cost\n");
    std::fprintf(stderr, "kmpp solution cost: %g\n", blz::sum(std::get<2>(kmppdat)));
    timer.restart("kmc");
    auto kmcdat = fgc::distance::make_kmc2(full_jsm, k, m);
    timer.report();
    ...
    auto oracle = [](const auto &x, const auto &y) {
        return std::sqrt(fgc::distance::multinomial_jsd(x, y));
    };
    ...
    auto full_jsd = fgc::distance::make_probdiv_applicator(filtered_sparsemat, fgc::distance::MKL);
    std::tie(centeridx, asn, costs) = fgc::distance::make_kmeanspp(full_jsd, k);
    centers = rows(filtered_sparsemat, centeridx.data(), centeridx.size());
    fgc::coresets::lloyd_loop(asn, counts, centers, filtered_sparsemat, 1e-6, 250, [](const auto &x, const auto &y) {return fgc::distance::multinomial_jsd(x, y);});
    //auto coreset_sampler = fgc::distance::make_d2_coreset_sampler(full_jsm, k, 13);
}
