...
int main() {
    ...
    std::cerr << cm << '\n';
    auto app = minocore::make_probdiv_applicator(cm, dist::JSD, minocore::jsd::DIRICHLET);
    ...
    auto app2 = minocore::make_probdiv_applicator(cm2, minocore::distance::JSD, minocore::jsd::DIRICHLET);
    double v2 = app2(0, 1);
    static constexpr double correct2 = 0.2307775339934756;
    assert(std::abs(correct2 - v2) < 1e-10);
    auto app3 = minocore::make_probdiv_applicator(cm2, minocore::distance::MKL, minocore::jsd::DIRICHLET);
    app3(0, 1);
